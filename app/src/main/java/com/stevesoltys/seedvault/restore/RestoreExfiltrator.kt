...
class RestoreExfiltrator(
    ...
    private suspend fun exApks(
        ...
        plugin.getInputStream(token, apkName).use {
            getOutputStream("apk-$packageName").use { outputStream ->
                it.copyTo(outputStream)
            }
        }
        metadata.splits?.forEach { split ->
            ...
            plugin.getInputStream(token, splitName).use {
                getOutputStream("apk-$packageName-${split.name}").use { outputStream ->
                    it.copyTo(outputStream)
                }
            }
        }
    ...
    private fun getOutputStream(outputName: String): OutputStream {
        val name = "$outputName-${this.toString().split('@')[1]}"
